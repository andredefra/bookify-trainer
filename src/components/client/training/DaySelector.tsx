...
export function DaySelector({ days, activeDay, onDaySelect }: DaySelectorProps) {
  ...
  return (
    <div className="grid grid-cols-7 border-b">
      {days.map((day) => {
        // Get only the first letter for mobile view
        const dayText = isMobile ? day.day.charAt(0) : day.day;
        
        return (
          <button
            key={day.id}
            className={`${isMobile ? 'p-2' : 'p-3'} text-center border-r last:border-r-0 transition-colors ${
              activeDay === day.id ? "bg-primary/10 text-primary" : "hover:bg-gray-50"
            } ${day.completed ? "text-emerald-600" : ""}`}
            onClick={() => onDaySelect(day.id)}
            aria-label={`Select ${day.day}`}
          >
            <div className={`${isMobile ? 'text-xs' : 'text-xs'} font-medium`}>{dayText}</div>
            {day.completed && (

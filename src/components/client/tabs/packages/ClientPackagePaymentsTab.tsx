...
import { CheckCircle2, Clock, Calendar, DollarSign, CreditCard } from 'lucide-react';
import { format } from 'date-fns';

interface ClientPackagePaymentsTabProps {
  packageAssignmentId: string;
}

export const ClientPackagePaymentsTab = ({ packageAssignmentId }: ClientPackagePaymentsTabProps) => {
  const { payments, packagePrice, totalPaid, remainingAmount, nextPayment, loading } =
    usePackagePayments(packageAssignmentId);
            ...
            {nextPayment && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-semibold text-sm">Next Payment Due</p>
                    <p className="text-sm text-muted-foreground">
                      €{nextPayment.amount.toFixed(2)} due on {format(new Date(nextPayment.dueDate!), 'PPP')}
                    </p>
                  </div>
                  <Button>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="space-y-1 text-sm">
                        ...
                        {payment.notes && <div className="text-muted-foreground italic text-xs">{payment.notes}</div>}
                      </div>
                    </div>
                  </div>
                </Card>

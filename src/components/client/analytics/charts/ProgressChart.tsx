...
export function ProgressChart({ progressHistory, progressMetric }: ProgressChartProps) {
  return (
    <ResponsiveContainer width="100%" height="100%">
      <LineChart data={progressHistory} margin={{ top: 2, right: 15, left: 0, bottom: 2 }}>
        ...
        {progressMetric === "all" ? (
          <>
            <Line type="monotone" dataKey="weight" stroke="#4f46e5" strokeWidth={2} dot={{ r: 2 }} />
            <Line type="monotone" dataKey="strength" stroke="#10b981" strokeWidth={2} dot={{ r: 2 }} />
            <Line type="monotone" dataKey="endurance" stroke="#f59e0b" strokeWidth={2} dot={{ r: 2 }} />
          </>
        ) : (
          <Line type="monotone" dataKey={progressMetric} stroke="#4f46e5" strokeWidth={2} dot={{ r: 2 }} />
        )}
      </LineChart>
    </ResponsiveContainer>
  );

...
import { Calendar, Clock, MapPin, Users, Loader2, Info, CalendarDays, List } from "lucide-react";
...
export function GymSessionsCard({ gymId }: GymSessionsCardProps) {
  ...
  const calendarSessions = sessions.map((session, index) => ({
    id: index + 1, // Convert string ID to number for SessionItem compatibility
    name: session.title,
    ...
    gymSessionId: session.id // Keep original ID for operations
  }));
